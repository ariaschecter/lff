    ...
    <div class="col-md-12 grid-margin stretch-card col-lg-6">
        <div class="card ">
            ...
            <h6 class="card-title">Add Course</h6>
                <form method="POST" action="{{ url('admin/course_sub_list/'.$course->slug.'/update/'.$data->id) }}">
                    @csrf
                    <div class="mb-1">
                        <label for="sub_list_no" class="form-label">No</label>
                        <input type="number" class="form-control @error('sub_list_no') is-invalid @enderror" id="sub_list_no" name="sub_list_no" value="{{ $data->sub_list_no }}" placeholder="Input Sub Course No">
                    </div>
                    @error('sub_list_no') <div class="text-danger">{{ $message }}</div> @enderror
                    <div class="mb-1">
                        <label for="sub_list_name" class="form-label">Name</label>
                        <input type="text" autofocus class="form-control @error('sub_list_name') is-invalid @enderror" id="sub_list_name" name="sub_list_name" value="{{ $data->sub_list_name }}" placeholder="Input Sub Course Name">
                    </div>
